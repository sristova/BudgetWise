...
// ─── Goals Screen — BudgetWise ────────────────────────────────────────────────
// Features: list, progress, create, edit, delete, contribute, empty state,
//           loading state, pull-to-refresh, status badges, overdue detection.
...
import { goalsApi } from '@/lib/api';         // <- merge goalsApi into your existing api.ts
import { useAuth } from '@/contexts/AuthContext';
import type { Goal, CreateGoalPayload, UpdateGoalPayload, GoalStatus,} from '../../types/goals';

// ─── Design tokens (identical to rest of app) ─────────────────────────────────
const C = {
  ...
  warning: '#7A5A1A', warningText: '#F0C060',
  inputBg: '#0F0710',
  paused: '#3D2A10', pausedText: '#C4A060',
};
...
function statusLabel(goal: Goal): { label: string; bg: string; color: string } | null {
  ...
  return (
    <>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          {ICONS.map(ic => (
            <TouchableOpacity
              ...
              style={[S.iconBtn, fields.icon === ic && S.iconBtnActive]}
            >
      ...
      <TextInput
        ...
        placeholder="npr. Počitnice v Španiji"
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
        <View style={{ flexDirection: 'row', gap: 8 }}>
          {CURRENCIES.map(cur => (
            <TouchableOpacity
              key={cur}
              onPress={() => onChange({ currency: cur })}
              style={[S.currencyBtn, fields.currency === cur && S.currencyBtnActive]}
            >
...
function AddGoalModal({ visible, onClose, onSaved }: {
  ...
  function merge(partial: Partial<GoalFormFields>) {
  ...
  async function handleSave() {
    ...
    // Basic deadline format validation
    if (fields.deadline && !/^\d{4}-\d{2}-\d{2}$/.test(fields.deadline)) {
      return Alert.alert('Napaka', 'Rok mora biti v obliki LLLL-MM-DD.');
    }
    ...
    try {
      const payload: CreateGoalPayload = {
        ...
        ...(fields.deadline.trim() && { deadline: fields.deadline.trim() }),
      };
      ...
      <KeyboardAvoidingView style={{ flex: 1, justifyContent: 'flex-end' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={S.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            <GoalForm fields={fields} onChange={merge} />
          </ScrollView>
          ...
          >
            {saving
              ...
              : <Text style={S.primaryBtnText}>Shrani cilj</Text>
            }
          </TouchableOpacity>
        </View>
...
function EditGoalModal({ goal, onClose, onSaved }: {
  goal: Goal | null; onClose: () => void; onSaved: () => void;
}) {
  const [fields, setFields] = useState<GoalFormFields>(EMPTY_FORM);
      ...
      <KeyboardAvoidingView style={{ flex: 1, justifyContent: 'flex-end' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={S.sheet}>
          ...
          >
            {saving
              ? <ActivityIndicator color={C.text1} size="small" />
              : <Text style={S.primaryBtnText}>Shrani spremembe</Text>
            }
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
...
function ContributeModal({ goal, onClose, onSaved }: {
  ...
  async function handleContribute() {
    ...
    if (num > remaining + 0.01) {
      return Alert.alert('Napaka', `Maksimalni prispevek je ${formatCurrency(remaining, goal!.currency)}.`);
    }
    ...
    try {
      ...
      onClose();
      if (result?.completed) {
        setTimeout(() => Alert.alert('🎉 Čestitke!', `Cilj "${goal!.name}" je dosežen!`), 300);
      }
    } catch (err: any) {
        ...
        <View style={S.sheet}>
          ...
          <View style={{ backgroundColor: C.bg1, borderRadius: 12, padding: 12, marginBottom: 20, borderWidth: 0.5, borderColor: C.border1 }}>
            ...
            <Text style={{ fontSize: 12, color: C.text3, marginTop: 4 }}>
              Prihranili: {formatCurrency(Number(goal.currentAmount), goal.currency)} / {formatCurrency(Number(goal.targetAmount), goal.currency)}
            </Text>
            <Text style={{ fontSize: 12, color: C.warm, marginTop: 2 }}>
              Še manjka: {formatCurrency(remaining, goal.currency)}
            </Text>
          </View>
          ...
          <TextInput
            ...
            placeholder={`Max ${formatCurrency(remaining, goal.currency)}`}
            placeholderTextColor={C.text3}
            keyboardType="decimal-pad"
            autoFocus
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 20 }}>
            {[10, 25, 50, 100].map(q => (
              <TouchableOpacity
                key={q}
                onPress={() => setAmount(String(Math.min(q, Math.ceil(remaining))))}
                style={S.quickBtn}
              >
                <Text style={{ fontSize: 13, color: C.text2 }}>
                  {formatCurrency(q, goal.currency).replace(/\s/g, '\u00A0')}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          >
            {saving
              ? <ActivityIndicator color={C.text1} size="small" />
              : <Text style={S.primaryBtnText}>Dodaj prispevek</Text>
            }
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
...
function GoalCard({ goal, onContribute, onEdit, onDelete }: {
  ...
  const progress = goal.targetAmount > 0
    ? Math.min(Number(goal.currentAmount) / Number(goal.targetAmount), 1)
    : 0;
  const percentage = Math.round(progress * 100);
  const isCompleted = goal.status === 'COMPLETED';
  const overdue = isOverdue(goal);
  const badge = statusLabel(goal);
    ...
    <View style={[S.card, { borderColor: cardBorderColor }]}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {goal.description ? (
            <Text style={{ fontSize: 12, color: C.text3, marginTop: 1 }} numberOfLines={1}>{goal.description}</Text>
          ) : null}
          {goal.deadline ? (
            <Text style={{ fontSize: 12, color: overdue ? C.warningText : C.text3, marginTop: 1 }}>
              Rok: {new Date(goal.deadline).toLocaleDateString('sl-SI')}
            </Text>
          ) : null}
        </View>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8, marginBottom: 12 }}>
        <Text style={{ fontSize: 13, color: C.text1, fontWeight: '500' }}>
          {formatCurrency(Number(goal.currentAmount), goal.currency)}
        </Text>
        <Text style={{ fontSize: 13, color: C.text3 }}>
          cilj: {formatCurrency(Number(goal.targetAmount), goal.currency)}
        </Text>
      </View>
      ...
      {isCompleted ? (
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity
            ...
            style={[S.primaryBtn, { flex: 1, paddingVertical: 10 }]}
          >
...
function SummaryCard({ goals }: { goals: Goal[] }) {
  ...
  return (
    <View style={{ backgroundColor: C.accent, marginHorizontal: 16, marginBottom: 16, borderRadius: 16, padding: 16 }}>
      <Text style={{ fontSize: 11, color: 'rgba(245,238,232,0.7)', letterSpacing: 0.08, marginBottom: 4 }}>
        SKUPNI NAPREDEK
      </Text>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 10 }}>
        <Text style={{ fontSize: 28, fontWeight: '500', color: C.text1 }}>
          {formatCurrency(totalSaved)}
        </Text>
        <Text style={{ fontSize: 14, color: 'rgba(245,238,232,0.75)' }}>
          od {formatCurrency(totalTarget)}
        </Text>
      </View>
      ...
      <Text style={{ fontSize: 12, color: 'rgba(245,238,232,0.75)', marginTop: 6 }}>
        {pct}% skupnega cilja · {active.length} aktivnih
      </Text>
    </View>
  );
}
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <View style={{ alignItems: 'center', paddingTop: 60, paddingHorizontal: 40 }}>
      ...
      <Text style={{ fontSize: 17, fontWeight: '500', color: C.text1, marginBottom: 8, textAlign: 'center' }}>
        Ni nastavljenih ciljev
      </Text>
      <Text style={{ fontSize: 14, color: C.text3, textAlign: 'center', lineHeight: 21, marginBottom: 28 }}>
        Dodajte prvi varčevalni cilj in{'\n'}začnite slediti napredku.
      </Text>
      <TouchableOpacity
        onPress={onAdd}
        style={{ backgroundColor: C.accent, borderRadius: 12, paddingHorizontal: 24, paddingVertical: 13, flexDirection: 'row', alignItems: 'center', gap: 8 }}
      >
...
export default function GoalsScreen() {
  ...
  function handleDelete(goal: Goal) {
    Alert.alert(
      'Izbriši cilj',
      `Ali res želite izbrisati "${goal.name}"?`,
      [
        { text: 'Prekliči', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Napaka', 'Cilja ni bilo mogoče izbrisati.');
              fetchGoals(true); // revert
            }
          },
        },
      ],
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg2 }}>
      ...
      <View style={S.header}>
        <View>
          ...
          <Text style={{ fontSize: 12, color: C.text3, marginTop: 2 }}>
            {activeGoals.length} aktivnih · {completedGoals.length} doseženih
          </Text>
        </View>
        <TouchableOpacity
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ alignItems: 'center', paddingTop: 60, paddingHorizontal: 32 }}>
            ...
            <TouchableOpacity onPress={() => fetchGoals()} style={S.primaryBtn}>
              <Text style={S.primaryBtnText}>Poskusi znova</Text>
            </TouchableOpacity>
          </View>
      ...
      <AddGoalModal
        ...
        onSaved={() => fetchGoals(true)}
      />
      <EditGoalModal
        goal={editGoal}
        onClose={() => setEditGoal(null)}
        onSaved={() => { fetchGoals(true); setEditGoal(null); }}
      />
      <ContributeModal
        goal={contributeGoal}
...
// ─── Stylesheet ────────────────────────────────────────────────────────────────
// Kept as plain objects (same as rest of codebase — no StyleSheet.create to stay
// consistent with the inline pattern already used throughout the app).
...
const S = {
  ...
  iconContainer: {
    width: 44, height: 44, borderRadius: 12, backgroundColor: '#1A0D14',
    alignItems: 'center' as const, justifyContent: 'center' as const,
  },
  progressTrack: {
  ...
  progressFill: {
    height: '100%' as const, borderRadius: 6,
  },
  ...
  // Form
  label: {
    fontSize: 11, color: C.text3, textTransform: 'uppercase' as const, marginBottom: 6,
  },
  input: {
